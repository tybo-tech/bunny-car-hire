...
export class ProductService {
  ...
  adjustStockAfterSale(order: Order, product: Product = null) {
    if (!product)
      return;

    product.TotalStock = Number(product.TotalStock) - 1;
    this.update(product).subscribe(data => {
      console.log(data);
    })
  }
  ...
  increaseStock(product: Product, number: number) {
    if (!product)
      return;

    product.TotalStock = Number(product.TotalStock) + number;
    this.update(product).subscribe(data => {
      console.log(data);
    })
  }
